...
\item \textbf{Encoding}: It understands UTF-8 and uses it to determine word boundaries, and case-insensitive matching of a link reference label (Unicode case folding). However, it will \textit{not} translate HTML entities and numeric character references.
\end{enumerate}
...
Note that in the generated (La)TeX manuscript, the align information of the columns are ignored, and default to left align. It may be supported in the future.\par
