...
author: "李睿远"
date: "May 22, 2026"
description: "浏览器文件传输：从分片到 WebRTC 与离线方案"
latex: true
pdf: true
---
...
当网络不可用时，浏览器可借助 `IndexedDB` 或 `Cache Storage` 实现本地文件暂存。开发者先将文件转换为 `Blob`，再写入 `IndexedDB` 的对象存储；待网络恢复后，再从数据库读出并执行上传。`Service Worker` 可拦截网络请求，结合 `Background Sync` API，在恢复连接时自动重试失败的上传任务，实现近似原生的离线体验。
