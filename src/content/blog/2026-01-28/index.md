---
title: "纯 C 实现的机器学习模型推理"
author: "黄梓淳"
date: "Jan 28, 2026"
description: "纯 C 机器学习推理，从数学基础到高效部署"
latex: true
pdf: true
---
...
机器学习模型推理的核心是线性代数运算和非线性激活函数。向量和矩阵是基础数据结构，向量加法简单对应元素级运算，而矩阵乘法则遵循公式 $\mathbf{C} = \mathbf{A} \mathbf{B}$，其中 $C_{ij} = \sum_k A_{ik} B_{kj}$。在 C 中，我们用一维 float 数组模拟多维张量，避免复杂的多维数组库。转置操作则通过索引重映射实现，例如对于矩阵 $A$ 的转置 $A^T_{ij} = A_{ji}$。
...
INT8 量化用 per-tensor scale：预计算 min/max，推理 GEMM 用 int32 累加器后 dequant。Transformer 自注意力 $Attention(Q,K,V) = softmax(QK^T / \sqrt{d}) V$，小模型用 GEMM 实现 QKV 投影和 scaled dot-product。
