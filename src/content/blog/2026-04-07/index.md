...
请求设备权限是使用 WebUSB 的第一步。通过 `navigator.usb.requestDevice()` 方法，开发者可以弹出浏览器原生的设备选择对话框。这个方法接受一个选项对象，其中 `filters` 参数至关重要。它是一个数组，每项包含 `vendorId`、`productId` 或 `serialNumber` 等过滤器，用于匹配目标设备。例如，以下代码请求特定厂商的设备：
