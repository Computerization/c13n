...
author: "杨岢瑞"
date: "Aug 13, 2026"
description: "垃圾回收三要素与主流 JVM/Go 运行时工程实践"
latex: true
pdf: true
---
...
垃圾回收器的核心任务可以概括为三个相互关联的环节：识别无用对象、执行回收策略以及管理内存碎片。识别无用对象主要依赖可达性分析或引用计数两种方法。可达性分析从一组根对象出发，沿着引用链遍历所有可到达的对象，未被标记的对象即被判定为垃圾。引用计数则为每个对象维护一个计数器，当计数器归零时立即回收。
