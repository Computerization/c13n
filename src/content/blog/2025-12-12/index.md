---
title: "Tokenization 在 NLP 中的工作原理"
author: "杨子凡"
date: "Dec 12, 2025"
description: "NLP Tokenization 原理、算法与 Python 实战详解"
latex: true
pdf: true
---
...
这段代码模拟 BPE 训练：`texts.split()` 分词成字符列表，循环统计相邻对频率，选最高频合并并记录在 `merges` 字典。合并后用贪婪方式应用到所有词，确保子词一致。这个简化版忽略了完整词汇表构建，但捕捉了核心迭代逻辑。在 Hugging Face 中，你可比较不同 tokenizer：`tokenizer("Hello")` 输出差异揭示算法特性，如 BPE 更倾向子词拆分。BPE 的合并过程就像逐步构建拼图，高效捕捉语言规律。你最喜欢哪种方法，为什么？

## Tokenization 在 NLP 模型中的作用
