---
title: "通用、普通的技术主题：文件压缩算法"
author: "杨其臻"
date: "Jul 18, 2026"
description: "文件压缩算法全景：从熵理论到工程选型与实践策略"
latex: true
pdf: true
---
...
行程长度编码是最早被广泛采用的简单方法。它将连续重复的符号替换为「符号 + 计数」的二元组，例如对序列 `AAAAABBB` 可编码为 `A5B3`。该方法在扫描图像或传真数据中效果显著，但面对随机性较强的内容时几乎无法带来收益。
...
当数据集合中存在大量相似文件时，可通过预训练字典或块级重复数据删除进一步提升效果。Zstandard 的字典训练接口允许从样本集中学习高频短语，显著改善小文件压缩率。
