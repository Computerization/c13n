---
title: "Linux 文件系统目录结构标准"
author: "黄京"
date: "Nov 20, 2025"
description: "Linux 文件系统目录结构标准解析"
latex: true
pdf: true
---
...
`/dev` 目录体现了 Linux「一切皆文件」的哲学，它将硬件设备抽象为文件形式，允许用户通过文件操作与设备交互。例如，`/dev/sda` 代表第一个 SATA 硬盘，而 `/dev/null` 是一个特殊的空设备，任何写入其中的数据都会被丢弃，常用于屏蔽命令输出；`/dev/zero` 则提供无限的零字节流，常用于初始化或测试。这些设备文件不是普通文件，而是内核提供的接口，使得输入输出操作统一而简洁。
