...
\end{lstlisting}
此代码加载预训练 BERT 模型，对句子「bank of the river」进行编码。\texttt{last\_{}hidden\_{}state} 输出包含每个 token 的上下文相关向量，其中「bank」的向量会根据「river」的上下文动态调整，从而区别于「bank account」中的「bank」。\par
\chapter{词嵌入的应用实践}
\section{基础 NLP 任务}
...
\end{lstlisting}
此处 \texttt{vector\_{}size} 定义词向量维度，\texttt{window} 控制上下文窗口大小。通过 \texttt{np.mean} 对词向量取平均，得到句子级表示。\par
\section{高级应用场景}
...
plt.show()
\end{lstlisting}
此代码将高维词向量投影到二维平面，\texttt{n\_{}components=2} 指定输出维度为 2。可视化结果可清晰展示「国王-女王-男人-女人」的性别语义轴与「巴黎-法国」的地理关联。\par
\chapter{挑战与优化方向}
