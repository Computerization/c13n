...
\chapter{优化技巧与常见陷阱}
\section{工程优化实践}
哈希桶尺寸应动态扩展至 $\lfloor \log _\phi n \rfloor + 1$ 以避免重复分配。内存管理方面，可采用对象池缓存已删除节点，减少内存分配开销。在 \texttt{consolidate} 操作中，预计算最大度数 $D(n) = \lfloor \log _\phi n \rfloor$ 可精确控制桶数组大小。\par
\section{高频错误防范}
