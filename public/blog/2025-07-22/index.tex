\title{二叉搜索树（Binary Search Tree）—— 从理论到代码实践}
\author{杨子凡}
\date{Jul 22, 2025}
\maketitle
...
\chapter{复杂度分析与性能陷阱}
BST 的性能高度依赖于树的平衡性。理想情况下，随机数据构建的 BST 高度接近 $\log _2 n$，此时查找、插入、删除操作时间复杂度均为 $O(\log n)$。最坏情况（如输入有序序列）会退化成链表，高度 $h = n$，操作复杂度恶化至 $O(n)$。\par
