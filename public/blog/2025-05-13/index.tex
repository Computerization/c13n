...
头节点（Head）作为各层链表的起点，不存储实际数据，仅提供遍历入口。尾节点（Tail）通常为 \InlineCode{None}，标识链表的结束。这种设计使得跳表的查询过程可以从高层快速缩小范围，逐步下沉到底层定位目标。\par
...
\chapter{跳表的核心操作原理}
\section{查询操作}
查询操作的逻辑可概括为“从高层向底层逐级下沉”。以查找值 \InlineCode{target} 为例：\par
\begin{itemize}
\item 从最高层头节点出发，向右遍历直至当前节点的后继节点值大于 \InlineCode{target}。
\item 下沉到下一层，重复上述过程直至到达底层。
\item 最终检查底层节点的值是否等于 \InlineCode{target}。
\end{itemize}
...
\end{lstlisting}
\InlineCode{Node} 类的 \InlineCode{forward} 数组存储该节点在各层的后继指针。\InlineCode{SkipList} 类的 \InlineCode{max\_{}level} 限制最大层数以防止内存过度消耗，\InlineCode{p} 控制层数生成概率。\par
\section{随机层数生成}
\begin{lstlisting}[language=python]
...
\end{lstlisting}
\InlineCode{update} 数组保存了每层中最后一个小于待插入值的节点。插入新节点时，需从底层到新节点的最高层更新这些节点的指针。\par
\section{查询方法实现}
\begin{lstlisting}[language=python]
...
\section{优化方向}
\begin{itemize}
\item \textbf{动态调整最大层数}：根据数据规模自适应调整 \InlineCode{max\_{}level}，避免内存浪费。
